...
func TestKubernetesListNodeHeapsterStats(t *testing.T) {
	Convey("Kubernetes ListNodeHeapsterStats works correctly", t, func() {
		table := []struct {
			// Input
			kube *model.Kube
			// Mocks
			mockListNodeHeapsterStatsResponseCode int
			mockListNodeHeapsterStatsResponseBody string
			// Expectations
			items []*kubernetes.HeapsterStats
			err   error
		}{
			// A successful example
			{
				...
				kube: &model.Kube{},
				// Mocks
				mockListNodeHeapsterStatsResponseCode: 200,
				mockListNodeHeapsterStatsResponseBody: `[
          {
            "name": "Node1",
            "cpuUsage": 100,
            "memUsage": 200
			...
			// On error
			{
				...
				kube: &model.Kube{},
				// Mocks
				mockListNodeHeapsterStatsResponseCode: 500,
				mockListNodeHeapsterStatsResponseBody: `something bad`,
				// Expectations
				err: errors.New("K8S 500 error: something bad"),
			},
		}
		...
		for _, item := range table {
				...
				HTTPClient: &http.Client{
					Transport: &fake_http.RoundTripper{
						RoundTripFn: func(r *http.Request) (resp *http.Response, err error) {
							...
							if r.Method == "GET" && r.URL.Path == "/api/v1/proxy/namespaces/kube-system/services/heapster/api/v1/model/nodes" {
								resp = &http.Response{
									Status:     strconv.Itoa(item.mockListNodeHeapsterStatsResponseCode),
									StatusCode: item.mockListNodeHeapsterStatsResponseCode,
									Body:       ioutil.NopCloser(bytes.NewBufferString(item.mockListNodeHeapsterStatsResponseBody)),
								}
							} else {
								panic("Did not recognize request Method / URL Path: " + r.Method + " " + r.URL.Path + " " + r.URL.Query().Encode())
							}
			...
			items, err := kubernetes.ListNodeHeapsterStats()
