...
func TestNodesList(t *testing.T) {
	...
	Convey("UI Nodes List works correctly", t, func() {

		table := []struct {
			// Mocks
			// mockClientListItems []*model.Node
			...
			// A successful example
			{
				...
				responseURL:        "http://localhost:10000/ui/nodes",
			},
			// Unauthenticated
			{
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/nodes", nil)
...
func TestNodesNew(t *testing.T) {
	srv := newTestServer()
	go srv.Start()
	defer srv.Stop()

	Convey("UI Nodes New works correctly", t, func() {
		...
		table := []struct {
			...
			// A successful example
			{
				mockAuthenticated:  true,
				responseStatusCode: 200,
				responseURL:        "http://localhost:10000/ui/nodes/new",
			},
			// Unauthenticated
			{
				mockAuthenticated:  false,
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/nodes/new", nil)
...
func TestNodesCreate(t *testing.T) {
	...
	Convey("UI Nodes Create works correctly", t, func() {
		...
		table := []struct {
			...
			modelPassedToCreate *model.Node
			responseStatusCode  int
			responseURL         string
		}{
			// A successful example
		...
		for _, item := range table {
			...
			var modelPassedToCreate *model.Node
			var redirected bool
							...
							Nodes: &fake_client.Nodes{
								Collection: fake_client.Collection{
									CreateFn: func(m model.Model) error {
										modelPassedToCreate = m.(*model.Node)
			...
			req, _ := http.NewRequest("POST", "http://localhost:10000/ui/nodes", body)
...
func TestNodesGet(t *testing.T) {
	...
	Convey("UI Nodes Get works correctly", t, func() {
		...
		table := []struct {
			...
			// A successful example
			{
				mockAuthenticated:  true,
				responseStatusCode: 200,
				responseURL:        "http://localhost:10000/ui/nodes/1",
			},
			// Unauthenticated
			{
				mockAuthenticated:  false,
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/nodes/1", nil)
