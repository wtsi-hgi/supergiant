...
func TestCloudAccountsList(t *testing.T) {
	...
	Convey("UI CloudAccounts List works correctly", t, func() {

		table := []struct {
			// Mocks
			// mockClientListItems []*model.CloudAccount
			...
			// A successful example
			{
				...
				responseURL:        "http://localhost:10000/ui/cloud_accounts",
			},
			// Unauthenticated
			{
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/cloud_accounts", nil)
...
func TestCloudAccountsNew(t *testing.T) {
	srv := newTestServer()
	go srv.Start()
	defer srv.Stop()

	Convey("UI CloudAccounts New works correctly", t, func() {
		...
		table := []struct {
			...
			// A successful example
			{
				mockAuthenticated:  true,
				responseStatusCode: 200,
				responseURL:        "http://localhost:10000/ui/cloud_accounts/new",
			},
			// Unauthenticated
			{
				mockAuthenticated:  false,
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/cloud_accounts/new", nil)
...
func TestCloudAccountsCreate(t *testing.T) {
	...
	Convey("UI CloudAccounts Create works correctly", t, func() {
		...
		table := []struct {
			...
			modelPassedToCreate *model.CloudAccount
			responseStatusCode  int
			responseURL         string
		}{
			// A successful example
          ...
          "credentials": {"token": "my-special-token"}
        }`,
				mockAuthenticated:     true,
		...
		for _, item := range table {
			...
			var modelPassedToCreate *model.CloudAccount
			var redirected bool
							...
							CloudAccounts: &fake_client.CloudAccounts{
								Collection: fake_client.Collection{
									CreateFn: func(m model.Model) error {
										modelPassedToCreate = m.(*model.CloudAccount)
			...
			req, _ := http.NewRequest("POST", "http://localhost:10000/ui/cloud_accounts", body)
...
func TestCloudAccountsGet(t *testing.T) {
	...
	Convey("UI CloudAccounts Get works correctly", t, func() {
		...
		table := []struct {
			...
			// A successful example
			{
				mockAuthenticated:  true,
				responseStatusCode: 200,
				responseURL:        "http://localhost:10000/ui/cloud_accounts/1",
			},
			// Unauthenticated
			{
				mockAuthenticated:  false,
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/cloud_accounts/1", nil)
