...
func TestVolumesList(t *testing.T) {
	...
	Convey("UI Volumes List works correctly", t, func() {

		table := []struct {
			// Mocks
			// mockClientListItems []*model.Volume
			...
			// A successful example
			{
				...
				responseURL:        "http://localhost:10000/ui/volumes",
			},
			// Unauthenticated
			{
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/volumes", nil)
...
func TestVolumesNew(t *testing.T) {
	srv := newTestServer()
	go srv.Start()
	defer srv.Stop()

	Convey("UI Volumes New works correctly", t, func() {
		...
		table := []struct {
			...
			// A successful example
			{
				mockAuthenticated:  true,
				responseStatusCode: 200,
				responseURL:        "http://localhost:10000/ui/volumes/new",
			},
			// Unauthenticated
			{
				mockAuthenticated:  false,
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/volumes/new", nil)
...
func TestVolumesCreate(t *testing.T) {
	...
	Convey("UI Volumes Create works correctly", t, func() {
		...
		table := []struct {
			...
			modelPassedToCreate *model.Volume
			responseStatusCode  int
			responseURL         string
		}{
			// A successful example
		...
		for _, item := range table {
			...
			var modelPassedToCreate *model.Volume
			var redirected bool
							...
							Volumes: &fake_client.Volumes{
								Collection: fake_client.Collection{
									CreateFn: func(m model.Model) error {
										modelPassedToCreate = m.(*model.Volume)
			...
			req, _ := http.NewRequest("POST", "http://localhost:10000/ui/volumes", body)
...
func TestVolumesGet(t *testing.T) {
	...
	Convey("UI Volumes Get works correctly", t, func() {
		...
		table := []struct {
			...
			// A successful example
			{
				mockAuthenticated:  true,
				responseStatusCode: 200,
				responseURL:        "http://localhost:10000/ui/volumes/1",
			},
			// Unauthenticated
			{
				mockAuthenticated:  false,
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/volumes/1", nil)
...
func TestVolumesEdit(t *testing.T) {
	srv := newTestServer()
	go srv.Start()
	defer srv.Stop()

	Convey("UI Volumes Edit works correctly", t, func() {
		...
		table := []struct {
			...
			// A successful example
			{
				mockAuthenticated:  true,
				responseStatusCode: 200,
				responseURL:        "http://localhost:10000/ui/volumes/1/edit",
			},
			// Unauthenticated
			{
				mockAuthenticated:  false,
		...
		for _, item := range table {
			...
			req, _ := http.NewRequest("GET", "http://localhost:10000/ui/volumes/1/edit", nil)
...
func TestVolumesUpdate(t *testing.T) {
	...
	Convey("UI Volumes Update works correctly", t, func() {
		...
		table := []struct {
			...
			// Expectations
			redirected          bool // how to distinguish between a successful create and a failure
			modelPassedToUpdate *model.Volume
			responseStatusCode  int
			responseURL         string
		}{
			// A successful example
		...
		for _, item := range table {
			...
			var modelPassedToUpdate *model.Volume
			var redirected bool
							...
							Volumes: &fake_client.Volumes{
								Collection: fake_client.Collection{
									UpdateFn: func(id interface{}, m model.Model) error {
										modelPassedToUpdate = m.(*model.Volume)
			...
			req, _ := http.NewRequest("POST", "http://localhost:10000/ui/volumes/1", body)
