...
func (db *DB) Create(r Collection, id string, m Resource) error {
	key := r.EtcdKey(id)
...
func (db *DB) Get(r Collection, id string, out Resource) error {
	key := r.EtcdKey(id)
	resp, err := db.get(key)
	if err != nil {
		return err
...
func (db *DB) Update(r Collection, id string, m Resource) error {
	key := r.EtcdKey(id)
...
func (db *DB) Delete(r Collection, id string) error {
	key := r.EtcdKey(id)
	_, err := db.delete(key)
	return err
}
...
func (db *DB) CompareAndSwap(r Collection, id string, old Resource, new Resource) error {
	key := r.EtcdKey(id)
